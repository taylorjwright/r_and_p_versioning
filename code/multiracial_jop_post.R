...
# tjw: needed to add these dependencies for Zelig
# install.packages(c("sandwich", "VGAM", "jsonlite", "AER", "plyr", "dplyr", "quantreg", "geepack", "MCMCpack", "maxLik", "Amelia", "MatchIt", "survey"))
...
# tjw: had to use this one not the above one
...
source("code/original/multiracial_jop_functions_robust.R")
...
df <- read.spss('data/raw/dataverse_files/multiracial.sav', to.data.frame = TRUE)
...
ggsave("paper/figures/figure1_postchange.png", width = 7, height = 2.5)
...
ggplot(m2w, aes(y = mean, x = model)) +
  ...
  facet_wrap(~ race) +
  ylab("Difference from Monoracial Whites") + xlab("")
ggsave("paper/figures/figure2_postchange.png", width = 7, height = 2.5)
