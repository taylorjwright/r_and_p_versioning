...
Figures \ref{dfifig1} and \ref{dfifig2} present Figures 1 and 2 from
@davenport2022multiracial which contain the main results. Panel A corresponds to 
results for White-Asian respondents while panel B corresponds to White-Black 
respondents. In Figure \ref{dfifig1}, the x-axis reflects the differences in a given 
multiracial group's attitudes relative to the corresponding monoracial group 
(e.g. White-Asian relative to Asians) with higher values meaning more liberal 
attitudes. In Figure \ref{dfifig2}, the x-axis reflects a similar difference but relative
to monoracial Whites (e.g. White-Asian relative to Whites). 
Figure 1 indicates that White-Asians have more liberal attitudes on cultural issues
than Asians while White-Blacks have more liberal attitudes on cultural issues 
than Blacks but views regarding social welfare and police perception appear to 
be comparable between White-Asians and Asians and between White-Blacks and Blacks. 
Figure 2 Panel A suggests that White-Asians are more liberal than Whites when it 
comes to cultural issues and police perceptions but similarly liberal with views 
on social welfare. Figure 2 Panel B indicates that White-Blacks are similarly 
liberal to Whites on cultural issues but are more liberal on issues of social 
welfare and police perceptions. 
...
itself. The code files contain comments noting that the analysis "Requires R 
...
Figures \ref{postfig1} and \ref{postfig2} present Figures 1 and 2 from 
@davenport2022multiracial when using a later software version (specifically a 
version of R post 3.6.0 such as the version 3.6.2 that the authors note in their 
code files). In Panel A of Figures \ref{postfig1} and \ref{postfig2}, 
White-Asians are now more liberal than Asians and Whites on all issues. In Panel 
B of \ref{postfig1} White-Blacks have more liberal views on cultural issues than 
Blacks (in terms of point estimates though the confidence interval now contains 0) 
and are now more liberal regarding police perceptions. In Panel B of \ref{postfig2} 
the point estimates appear very similar though there is an issue computing 
...
Figures \ref{postfig1} and \ref{postfig2} provide Figures 1 and 2 from 
@davenport2022multiracial when using an earlier software version (specifically a 
version of R pre 3.6.0). Minor issues with the confidence intervals in Panel B of
Figure \ref{prefig2} aside, these are otherwise the same as those presented in
the published article. 

The issue seems to be that the weights the authors use are non-integer and the 
main function used in their analysis, Zelig, uses sample() in cases with 
non-integer values (see
http://docs.zeligproject.org/articles/weights.html). Following changes to base R 
in 3.6.x, the sample() function and thus now Zelig, yields different results in 
R versions later than 3.5.x. Before R 3.6.0 RNGkind(sample.kind = "Rounding") 
was the default behaviour but after 3.6.0 the sample function's new default 
behaviour is RNGkind(sample.kind = "Rejection") (see 
https://blog.revolutionanalytics.com/2019/05/whats-new-in-r-360.html for 
more details).

The key takeaway is that despite the authors providing replication materials and
indicating which version of R they believed they used, the results cannot be 
reproduced. This is not a criticism of the authors, but rather a demonstration
of the challenges of reproducibility in the face of software updates. There are 
ways to ensure that the estimates are consistent (such as ensuring to always 
...
*containers* instantiated from the image definition to run the output.
...
software used. Another important aspect is *readability*, which is difficult to
...
updated as the scripts evolve. When some parts of the code are changed in one
...
research paper (and in particular its computer code) as a piece of software.
...
allow one to describe how a particular piece of software should be built. Other
...
can detect that these targets are linked. The third target is 
`multiracial_1`, which is defined as the output of a function called `basic_clean_1()`.
This is a function that we defined in another script called `functions.R`. This 
...
as the output of the `closeness_2()` to function. Here again, readers who are
interested in what `closeness_2()` does can look at its source code. If not,
they can move on.
...
Here is the visual representation of the pipeline, before running it:

![A visual representation of the pipeline. No output has been computed yet. \label{fig_dag_before}](./figures/figure3_dag_before.png)
...
- two functions were defined that are not being used at all (bottom left).

This is actually an interactive visualization that opens in the user's web browser,
...
![Every output has now been computed. \label{fig_dag_after}](./figures/figure4_dag_after.png)
...
example, here we changed some code in one of the functions and visualize the
pipeline:

![We can now see that one of the outputs is out of date. \label{fig_dag_ood}](./figures/figure5_dag_ood.png)
...
![The graph opens in your web browser, and it is possible to zoom in. \label{fig_dag_zoom}](./figures/figure6_dag_zoom.png)
...
and then chain them one after the next to generate an output.
...
start by looking at the `_targets.R` file, and then visualize the pipeline and
...
One final important note to improve the reproducibility of a project: the output of
...
and by writing a pipeline using {targets} instead of using scripts, researchers
